...
It is TCP because socket() is created with SOCK_STREAM.

3. The client is going to send some data to the server. Where does it get this data from? How can
you tell in the code? The client reads data from standard input (the keyboard) using
read(STDIN_FILENO, buf, BUF_SIZE).

4. How does the client program end? How can you tell that in the code?
The client exits when read() from standard input returns 0 (EOF,
such as pressing Ctrl+D). After the loop it closes the socket with
close(sfd) and calls exit(EXIT_SUCCESS).
*/
